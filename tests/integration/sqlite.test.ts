...
function fixture() {
  ...
  const db = openSqliteDatabase(join(dir, "state.db")); let index = 0;
...
describe("SQLite adapter", () => {
  test("bootstraps the promised pragmas, schema objects, indexes, and immutable event triggers", () => {
    ...
    expect(raw.query("PRAGMA user_version").get()).toMatchObject({ user_version: 1 });
    ...
    for (const name of ["projects", "tasks", "events", "idx_tasks_project_created", "idx_tasks_project_planned", "idx_tasks_project_observed", "idx_events_project_sequence", "idx_events_aggregate_sequence", "idx_events_occurred", "events_immutable_update", "events_immutable_delete"]) expect(schema.some((entry) => entry.name === name)).toBe(true);
    raw.close();
  });
  ...
  test("rolls current back when event append fails and writes no event when current update fails", () => {
    ...
    raw.close();
    const currentFailure = openSqliteDatabase(path);
    ...
    eventRaw.close();
    const eventFailure = openSqliteDatabase(path);
  ...
  test("fails closed before DDL when database schema is newer", () => {
    const { dir, db } = fixture(); db.close(); const path = join(dir, "state.db"); const raw = new Database(path); raw.exec("PRAGMA user_version=2"); raw.close();
    try { openSqliteDatabase(path); throw new Error("expected schema rejection"); } catch (error: any) { expect(error.code).toBe("SCHEMA_TOO_NEW"); }
  });
  ...
  test("rolls a failed migration back without advancing user_version", () => {
    ...
    try { openSqliteDatabase(path); throw new Error("expected migration failure"); } catch (error: any) { expect(error.code).toBe("MIGRATION_FAILED"); }
  ...
  test("returns DB_BUSY rather than a version conflict while another connection holds the write lock", async () => {
    const { dir, db } = fixture(); db.close(); const path = join(dir, "state.db");
    const holder = new Database(path); holder.exec("BEGIN IMMEDIATE");
    const child = Bun.spawn([process.execPath, resolve("src/cli/main.ts"), "--db", path, "--json", "project", "add", "--name", "Busy", "--root", dir], { cwd: dir, stdout: "pipe", stderr: "pipe" });
    const [exit, stderr] = await Promise.all([child.exited, new Response(child.stderr).text()]);
    holder.exec("ROLLBACK"); holder.close();
    expect(exit).toBe(5);
    expect(JSON.parse(stderr)).toMatchObject({ ok: false, error: { code: "DB_BUSY", details: { timeoutMs: 5000 } } });
  }, 7000);
});
